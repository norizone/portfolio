...
export const ContactForm = () => {
  ...
  const onSend = async (e: React.FormEvent<HTMLButtonElement>) => {
    e.preventDefault()
    if (isSending) return
    const validation = formArgs.every(
      (el) => !el.require || (el.require && el.value && !el.hasError)
    )
    if (validation === true) {
      setIsSending(true)
      await onSendMail()
    } else {
      const emptyIndex: Array<number | null> = formArgs
        .map((el, index) => (el.require && !el.value ? index : null))
        .filter((index) => index !== null)
      emptyIndex.length > 0 &&
        emptyIndex.forEach((num) => {
  ...
  const onSendMail = async () => {
    ...
    await axios
      .post(process.env.NEXT_PUBLIC_SEND_MAIL_URL + '', sendData, {
      ...
      {!isSendSuccess && (
        <form>
          {formArgs.map((el, index) => (
            <div
              className={clsx(styles.form__field, isSending && styles.sending)}
              key={index}
            >
              <ContactInput
                {...el}

...
export const MockPic:FC<Props> = (props)=>{
  ...
  const setAnimationValues = () =>{
    ...
    executionScrollCount = Math.floor(
  ...
  const observerScroll = ()=> {
    ...
    new IntersectionObserver(
      scrollCallback, options).observe(
  ...
  return(
    ...
    <div className={styles["mockPic__inside-wrap"]}>
      <div className={clsx(styles["mockPic__inside"],styles[device])} 
      ref={autoScrollParentRef}
      >
        <div className={styles.mockPic__glass}></div>
        <picture
          className={styles["mockPic__pic"]}
          ref={autoScrollTargetRef}
        >
          <Image 
          src={images.url + '?fm=webp&q=60'}
